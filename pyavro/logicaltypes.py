...
class LogicalTypes:
        ...
        try:
            match type_name:
                case LogicalTypes.DECIMAL:
                    logical_type = LogicalTypes.Decimal.from_schema(schema)
    ...
    class Uuid(LogicalType):
        ...
        def validate(self, schema):
            ...
            if (schema.type != Schema.Type.STRING and schema.type != Schema.Type.FIXED):
                raise ValueError("Uuid can only be used with an underlying string or fixed type")
            if (schema.type == Schema.Type.FIXED and schema.get_fixed_size() != self.UUID_BYTES):
    ...
    class Duration(LogicalType):
        ...
        def validate(self, schema):
            super().validate(schema)
            if schema.type != Schema.Type.FIXED or schema.get_fixed_size() != 12:
    ...
    class Decimal(LogicalType):
        ...
        def validate(self, schema):
            super().validate(schema)
            if schema.type != Schema.Type.FIXED and schema.type != Schema.Type.BYTES:
            ...
            if self.precision < 0:
                ...
                if schema.type == Schema.Type.FIXED:
    ...
    class Date(LogicalType):
        ...
        def validate(self, schema: Schema):
            super().validate(schema)
            if schema.type != Schema.Type.INT:
    ...
    class TimeMicros(LogicalType):
        ...
        def validate(self, schema: Schema):
            super().validate(schema)
            if schema.type != Schema.Type.LONG:
